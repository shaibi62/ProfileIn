...
useEffect(() => {
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://localhost/Profilein-Backend/getUserData.php?id=${user?.id}`);
      if (response.data.success) {
        ...
        setFormData((prev) => ({
          ...
          education: (data.education || []).map(item => ({
            ...
            startYear: item.Start_Year || item.start_year || "",
            endYear: item.Completion_Year || item.completion_year || "",
            grade: item.grades || item.Grades || ""
          })),
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await axios.post(
      "http://localhost/Profilein-Backend/editUserInfo.php",
      formDataToSend,
      {
        headers: {
